...
$(document).ready(function(){
    ...
    $(document).on('click', '#pagination-container a', function(event){
        ...
        var page = $(this).attr('href').split('page=')[1]; // Obtiene el número de página
    ...
    function loadPaginatedData(page){
        $.ajax({
            ...
            method: 'GET',
            data: {page: page}, // Envía el número de página
            success: function(data){
                $('#lista-container').html(data); // Actualiza el contenedor de datos con los nuevos datos
                // Marcación de la página activa en los enlaces de paginación
                $('.pagination .page-item').removeClass('active'); // Elimina la clase 'active' de todos los enlaces de paginación
                $('.pagination .page-item').find('a').each(function(){
                    if ($(this).text() == page || (page == 1 && $(this).text() == '«')) {
                        $(this).parent().addClass('active'); // Agrega la clase 'active' al enlace correspondiente a la página actual
                    }
                });

                // Actualización de la URL del navegador
                var newUrl = window.location.href.split('?')[0] + '?page=' + page; // Construye la nueva URL con el número de página
                window.history.pushState({path: newUrl}, '', newUrl); // Actualiza la URL del navegador
            }
        });
    }
});
...
$('#search').on('keyup', function(){
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            $.each(data.data, function(key, usuario){
                var row = '<tr>';
